...
module.exports = class oriSocket
{
    constructor(config,dist)
	{
        ...
        wsServer.on('request', (request)=> {
            ...
            try{
                ...
                {
					// var con=this.config.connectService;
					// if(con)
					// {
						// await dist.run(con.domain,con.service,{key:request.key})
					// }
    ...
    async echoProtocolMessage(message,connection,key,dist)
	{
		...
		var session= await this.getSession(data,this);
		var body={ 
			data:data.param,
			key
		}
	...
	echoClose(connection,key,dist)
	{
		var dis=this.config.disconnectService;
		if(dis)
		{ 	 
			dist.run(dis.domain,dis.service,{key},(ee,dd)=>{  })
		}  
	}
}