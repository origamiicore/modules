...
class leaderboardManager
{
    ...
    getCount(gameId,func)
	{
        try{
            return this.disc.run('leaderboard','getCount',{gameId},func)
            
        }catch(exp){console.log(exp)}

	}
    getTop(gameId,top,func)
	{  
        try{
            return this.disc.run('leaderboard','getTop',{gameId,top},func)
...
module.exports = class leaderboardIndex
{
	...
	async getTop(msg,func,self)
	{
		//gameId,top
		var dt=msg;  
        self.connection.zrevrange(dt.gameId,  0, dt.top, 'withscores', async function (err, response) { 
	...
	async getCount(msg,func,self)
	{
		//gameId,top
		var dt=msg;  
        self.connection.zcount(dt.gameId,'-inf', '+inf', async function (err, response) { 
            if (err) return func({m:'leaderboard001'})
