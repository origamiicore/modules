...
class RedisRouting
{
	...
	GetSetMember(context,key,func)
	{
		return this.disc.run('redis','getSetMember',{context,name:key},func);
	}
	...
	RemoveUnique(context,key,value,func)
	{
		return this.disc.run('redis','arrayRemoveUnique',{context,name:key,value},func);
	}
	UniqueArrayLength(context,key,func)
	{
		return this.disc.run('redis','uniqueArrayLength',{context,name:key},func);
	}
	FindUniqueArray(context,key,value,func)
	{
		return this.disc.run('redis','findUniqueArray',{context,name:key,value},func);
	}
...
module.exports = class defaultIndex
{
	...
	async getSetMember(msg,func,self)
	{
		var dt=msg; 
		if(!self.context[dt.context])
			return func({m:"redis001"}) 
		
			self.context[dt.context].smembers(dt.name,(err,data)=>{				
				return func(err,data);
			}); 
	}
	...
	async arrayRemoveUnique(msg,func,self)
	{
		var dt=msg; 
		if(!self.context[dt.context])
			return func({m:"redis001"}) 
		self.context[dt.context].srem(dt.name,dt.value,(err,data)=>{
			
			return func(err,data);
		}); 
	}
	async uniqueArrayLength(msg,func,self)
	{
		var dt=msg; 
		if(!self.context[dt.context])
			return func({m:"redis001"}) 
		self.context[dt.context].scard(dt.name,(err,data)=>{
			
			return func(err,data);
		}); 
	}
	async findUniqueArray(msg,func,self)
	{
		var dt=msg; 
		if(!self.context[dt.context])
			return func({m:"redis001"}) 
		self.context[dt.context].smismember(dt.name,dt.value,(err,data)=>{
